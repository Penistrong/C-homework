...
/**绘制菜单包括其选择的功能*/
void PrintMenu() {
	choice:FlushConsoleInputBuffer(GetStdHandle(STD_INPUT_HANDLE));//清除之前玩家乱按键盘的操作
...
void PrintPauseMenu() {
	...
	settextcolor(WHITE);
	settextstyle(16, 8, _T("Comic Sans MS"));
	outtextxy(290, 190, _T("继续游戏"));
	outtextxy(290, 270, _T("保存并退出"));
	int PauseChoice = 1;
	int breakFlag = 0;
	while (TRUE) {
		if (_kbhit()) {
